...
const PnpmIcon = (p: SVGProps<SVGSVGElement>) => (
  ...
  >
    <defs>
      <path d="M237.6 95H187.6V45h50v50Z" id="a" />
      <path d="M182.59 95H132.59V45h50v50Z" id="b" />
      <path d="M127.59 95H77.59V45h50v50Z" id="c" />
      <path d="M237.6 150H187.6v-50h50v50Z" id="d" />
      <path d="M182.59 150H132.59v-50h50v50Z" id="e" />
      <path d="M182.59 205H132.59v-50h50v50Z" id="f" />
      <path d="M237.6 205H187.6v-50h50v50Z" id="g" />
      <path d="M127.59 205H77.59v-50h50v50Z" id="h" />
    </defs>
...
/**
 * Only the two grammars this site needs, rather than Shiki's `codeToHtml`,
 * whose bundled entry pulls in every language it ships. That was putting C++,
 * Emacs Lisp, Wolfram and ~200 others in front of visitors in order to
 * highlight two JavaScript snippets.
 *
 * Two rather than four because TypeScript is a superset of JavaScript and TSX
 * of JSX, so `js` and `jsx` are handled by aliasing rather than by loading
 ...
 * every grammar it is given, so building one per block would repeat that for
 * every code block on screen.
 */
const LANG_LOADERS = {
...
function getHighlighter() {
  ...
  // block first renders, rather than riding along in the route chunk that
  // every page load blocks on.
  highlighter ??= (async () => {
    ...
    return core.createHighlighterCore({
      ...
      langs: [],
      // The raw engine, paired with pre-compiled grammars. The default
      // Oniguruma engine ships a ~600KB WASM binary, and the non-raw
...
// A local addition to the registry component, which could only ever express
// `shadcn@latest add <url>`. Installing an ordinary npm package needs the
// plain add/install verbs instead.
