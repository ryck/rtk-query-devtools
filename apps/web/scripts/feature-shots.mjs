 ...
 *
 * Both themes are captured, into `public/features/{dark,light}/`, because the
 * site itself is themeable: a dark screenshot on a light page reads as a hole
 * in the layout. `Feature` picks the matching set.
 */
...
/**
 * One full pass over the demo for a single theme. The whole flow has to repeat
 * rather than just re-screenshotting, because the cache state is built up by
 * driving the app and does not survive a context swap.
 */
async function captureTheme(theme) {
  const outDir = `${OUT}/${theme}`
  ...
  // Seed the shell so the panel opens tall on load. Otherwise every shot needs a
  // click and comes out 290px high.
  await context.addInitScript((mode) => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        triggerHidden: true,
        plugins: [
          { name: "RTK Query", id: "rtk-query-devtools", defaultOpen: true },
        ],
      })
    )
  }, theme)
  ...
  await page
    .getByRole("button", { name: "Use regular expression search" })
    .click()
  ...
  console.log(
    errors.length ? `PAGE ERRORS: ${JSON.stringify(errors)}` : "No page errors."
  )
  await context.close()
}

for (const theme of ["dark", "light"]) {
  // Sequential on purpose: both passes drive the same demo server, and running
  // them at once would interleave the cache-seeding clicks.
  // eslint-disable-next-line no-await-in-loop
