...
import type { EndpointType, RtkQueryApiLike, TimelineEvent, TimelineOutcome } from "./types";
...
export class DevtoolsRegistry {
  ...
  configure(options: { apis?: RtkQueryApiLike[]; maxTimelineEntries?: number } = {}): void {
    if (options.apis) {
      for (const api of options.apis) this.#apis.set(api.reducerPath, api);
    }
    if (typeof options.maxTimelineEntries === "number" && options.maxTimelineEntries > 0) {
  ...
  getEndpointType(reducerPath: string, endpointName: string): EndpointType | undefined {
  ...
  recordAction(action: unknown): void {
    ...
    for (const reducerPath of this.getReducerPaths()) {
      ...
      if (phase !== "pending" && phase !== "fulfilled" && phase !== "rejected") continue;
  ...
  #handleThunkAction(
    ...
    if (existing) {
      ...
      existing.error = phase === "rejected" ? (action.payload ?? action.error) : undefined;
      this.scheduleNotify();
      return;
    }
    ...
    this.#pushTimeline({
      ...
      forceRefetch,
      subscribe,
      error: phase === "rejected" ? (action.payload ?? action.error) : undefined,
    });
  }
  ...
  #pushTimeline(event: TimelineEvent): void {
    ...
    if (this.#timeline.length > this.#maxTimelineEntries) {
      ...
      if (evicted && evicted.outcome === "pending") this.#pendingEvents.delete(evicted.requestId);
    }
    this.scheduleNotify();
  }
