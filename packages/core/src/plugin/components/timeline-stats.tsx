  ...
  return (
    <div className={clsx("rtkq:shrink-0 rtkq:border-b", classes.border)}>
      ...
      >
        ...
        <span
          className={clsx(
            "rtkq:ml-1 rtkq:font-normal rtkq:normal-case",
            classes.textDimmed
          )}
        >
          {stats.overall.count} requests · median{" "}
          {formatDuration(stats.overall.medianMs)} · slowest{" "}
  ...
  return (
    <>
      ...
      <div className={clsx("rtkq:tabular-nums", classes.textMuted)}>
        {stats.count}
        {stats.errorCount > 0 && (
          <span className={classes.danger}> ({stats.errorCount} err)</span>
        )}
      </div>
      {[stats.fastestMs, stats.medianMs, stats.averageMs, stats.slowestMs].map(
        (value, index) => (
          <div
