...
function read<T>(
  key: string,
  initial: T,
  codec: PersistCodec<T> | undefined
): T {
...
export function usePersistentState<T>(
  ...
  // on every render. Updated in an effect rather than during render: a ref
  // written mid-render is observable when React re-renders without committing.
  // Declared before the write effect below so that, on any commit where both
  // run, the codec is already current by the time the write reads it.
...
export function setCodec<T extends string>(
...
export function enumCodec<T extends string>(
  ...
  return {
    ...
    parse: (raw) =>
      typeof raw === "string" && allowed.includes(raw as T)
        ? (raw as T)
        : undefined,
  }
}
