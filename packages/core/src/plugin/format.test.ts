...
import { formatDuration, formatQueryCacheKey, formatRelativeTime, safeStringify } from "./format";

function namedFn() {}

describe("formatQueryCacheKey", () => {
  it("collapses the (undefined) suffix for genuinely no-arg queries", () => {
    expect(formatQueryCacheKey("listPosts(undefined)", undefined)).toBe("listPosts()");
  });
  ...
  it("does not touch a key that merely contains the text 'undefined' in real args", () => {
    ...
    expect(formatQueryCacheKey('getPost("undefined")', "undefined")).toBe('getPost("undefined")');
  });
});
...
describe("safeStringify", () => {
  it("stringifies plain values normally", () => {
    expect(safeStringify({ a: 1, b: "two" })).toBe(JSON.stringify({ a: 1, b: "two" }, null, 2));
  });
  ...
  it("serializes Date values via their native toJSON, no special-casing needed", () => {
    ...
    expect(JSON.parse(safeStringify({ date }))).toEqual({ date: "2026-01-01T00:00:00.000Z" });
  });

  it("serializes functions as a readable placeholder instead of dropping them silently", () => {
    expect(JSON.parse(safeStringify({ fn: namedFn }))).toEqual({ fn: "[Function: namedFn]" });
  });
