...
type RawProvided =
  | RawProvidedTags
  | { tags: RawProvidedTags; keys: RawProvidedKeys }
...
interface RawRtkQuerySlice {
  ...
  subscriptions: Record<
    string,
    Record<string, RawSubscriptionOptions> | undefined
  >
  config?: {
    reducerPath?: string
    online?: boolean
 ...
 */
export function selectApiHealth(
  state: unknown,
  reducerPath: string
): ApiHealth | undefined {
...
export function getRtkQuerySlice(
  ...
  if (!s.queries || !s.mutations || !s.provided || !s.subscriptions)
    return undefined
...
export function selectQueryEntries(
  ...
  for (const [queryCacheKey, sub] of Object.entries(slice.queries)) {
    ...
    const endpointName =
      sub.endpointName ?? parseEndpointNameFromCacheKey(queryCacheKey)
...
export function selectMutationEntries(
  state: unknown,
  reducerPath: string
): MutationEntry[] {
  const slice = getRtkQuerySlice(state, reducerPath)
  ...
  mutationEntriesCache.set(reducerPath, {
    mutationsRef: slice.mutations,
    result,
  })
  return result
}
...
export function selectTagGroups(
  state: unknown,
  reducerPath: string
): TagGroup[] {
  const slice = getRtkQuerySlice(state, reducerPath)
  ...
  for (const [tagType, byId] of Object.entries(provided.tags)) {
    const entries: TagGroupEntry[] = Object.entries(byId).map(
      ([id, queryCacheKeys]) => ({
        id,
