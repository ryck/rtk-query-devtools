...
describe("resetApiState", () => {
  it("dispatches the exact reset action type and clears all cached entries", async () => {
    ...
    expect(selectQueryEntries(store.getState(), api.reducerPath)).toHaveLength(1);

    recorder.actions.length = 0;
    resetApiState(registry, api.reducerPath);

    expect(recorder.actions.map((a) => a.type)).toContain(`${api.reducerPath}/resetApiState`);
    expect(selectQueryEntries(store.getState(), api.reducerPath)).toHaveLength(0);
  });
});
...
describe("removeQueryEntry", () => {
  it("dispatches the exact remove action type and removes only the targeted entry", async () => {
    ...
    expect(selectQueryEntries(store.getState(), api.reducerPath)).toHaveLength(2);
...
describe("removeMutationEntry", () => {
  it("dispatches the exact remove action type and removes an untracked-key entry by requestId", async () => {
    ...
    vi.mocked(fetch).mockResolvedValue(jsonResponse({ id: 2, title: "New" } satisfies Post));
    ...
    expect(selectMutationEntries(store.getState(), api.reducerPath)).toHaveLength(0);
  });
  ...
  it("uses the fixedCacheKey when the entry's cacheKey differs from its requestId", async () => {
    ...
    await store.dispatch(
      api.endpoints.addPost.initiate({ title: "New" }, { fixedCacheKey: "add-post" }),
    );
...
describe("refetch", () => {
  ...
  it("dispatches a forceRefetch initiate thunk when the api is registered", async () => {
    ...
    expect(refetch(registry, api.reducerPath, "getPost", 1)).toEqual({ ok: true });
    await vi.waitFor(() => expect(fetch).toHaveBeenCalledTimes(2));
