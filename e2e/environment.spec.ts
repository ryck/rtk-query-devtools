...
test("the offline toggle flips state and refetches on reconnect", async ({
  page,
}) => {
  await gotoApp(page)
  ...
  await switchTab(page, "Timeline")
  await expect
    .poll(async () => entryRow(page, "listPosts").count())
    .toBeGreaterThan(before)
})

test("the focus toggle flips state and refetches on refocus", async ({
  page,
}) => {
  await gotoApp(page)
  await openDevtoolsShell(page)
