...
test("Refetch triggers a new request for the selected entry", async ({
  page,
}) => {
  await gotoApp(page)
  ...
  await switchTab(page, "Timeline")
  await expect
    .poll(() => timelineEntryCount(page, "listPosts"))
    .toBeGreaterThan(before)
})

test("the detail pane lists every request made for the selected entry", async ({
  page,
}) => {
  await gotoApp(page)
  await openDevtoolsShell(page)
...
test("Invalidate tags refetches queries subscribed to that tag", async ({
  page,
}) => {
  await gotoApp(page)
  await openDevtoolsShell(page)
