...
test("the devtools shell opens and shows the RTK Query plugin tab", async ({
  page,
}) => {
  await gotoApp(page)
...
test("queries appear in the Queries tab and update as the app is used", async ({
  page,
}) => {
  await gotoApp(page)
  await openDevtoolsShell(page)
...
test("search filters the query list by endpoint name", async ({ page }) => {
  ...
  await page
    .getByPlaceholder("Search endpoint or args…")
    .fill("nonexistent-endpoint-xyz")
...
test("the active tab, search, and sort order survive a reload", async ({
  page,
}) => {
  await gotoApp(page)
  await openDevtoolsShell(page)
  ...
  await expect(
    page.getByRole("button", { name: "Sort order ascending" })
  ).toBeVisible()
  ...
  await switchTab(page, "Queries")
  await expect(page.getByPlaceholder("Search endpoint or args…")).toHaveValue(
    "listPosts"
  )
  await expect(
    page.getByRole("button", { name: "Sort order ascending" })
...
test("fuzzy search matches an acronym, which a substring filter would miss", async ({
  page,
}) => {
  await gotoApp(page)
  await openDevtoolsShell(page)
...
test("the API config strip summarises the api and expands on demand", async ({
  page,
}) => {
  await gotoApp(page)
  await openDevtoolsShell(page)
  ...
  await expect(
    page.getByText("keepUnusedDataFor", { exact: true })
  ).toBeVisible()
  await expect(
    page.getByText("invalidationBehavior", { exact: true })
  ).toBeVisible()
...
test("tab labels carry live counts and are wired to the panel for a11y", async ({
  page,
}) => {
  await gotoApp(page)
  await openDevtoolsShell(page)
  ...
  await expect
    .poll(async () => (await queries.textContent())?.trim())
    .toMatch(/Queries\s*\d+/)
...
test("the regex toggle filters by pattern and flags an invalid one", async ({
  page,
}) => {
  await gotoApp(page)
  await openDevtoolsShell(page)
  ...
  const toggle = page.getByRole("button", {
    name: "Use regular expression search",
  })
...
test("light and dark themes both render the panel without errors", async ({
  page,
}) => {
  const errors: string[] = []
